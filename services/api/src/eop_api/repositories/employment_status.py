...
class EmploymentStatusRepository(BaseRepository[EmploymentStatus]):
    """Data access layer for `EmploymentStatus`. Never commits or rolls back."""

    def __init__(self, session: AsyncSession) -> None:
        super().__init__(session, EmploymentStatus)

    async def get_by_code(self, code: str) -> EmploymentStatus | None:
        stmt = select(EmploymentStatus).where(EmploymentStatus.code == code)
