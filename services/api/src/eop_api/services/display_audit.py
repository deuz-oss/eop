...
from eop_api.repositories.display_audit import DisplayAuditRepository
...
class VisitNotFoundError(Exception):
    """Raised when the Visit referenced by a DisplayAudit does not exist."""


class DisplayAuditAuthorizationDeniedError(Exception):
    """Raised when the Visit Authorization Policy (Owner Only, evaluated
    against the DisplayAudit's *parent Visit*) denies a create/get/update/
    delete call -- i.e. `AuthorizationDecision.allowed` is `False`.

    Thrown only by `DisplayAuditService`, never by
    `VisitAuthorizationEvaluator` or `AuthorizationService` themselves.
    """


class DisplayAuditService:
    """Business logic for `DisplayAudit`. Owns the transaction boundary via
    a UoW.

    `DisplayAudit` is a repeatable display-compliance observation attached
    to a `Visit` -- many rows may reference the same Visit (`docs/
    architecture/capabilities/display-audit/
    display-audit-iteration-1-scope-and-implementation-plan.md` §2/§4),
    the same cardinality as `CompetitorActivity`/`PosmAudit`/`VisitPhoto`,
    ...
    *parent Visit* -- not a copy of `employee_id` on this entity itself,
    ...
    belongs to the caller's own `employee_id`, mirroring `PosmAuditService`
    exactly.
    ...
    async def create(
        self, data: DisplayAuditCreate, request_context: RequestContext
    ) -> DisplayAudit:
        async with self._uow_factory() as uow:
            ...
            repo = DisplayAuditRepository(uow.session)
    ...
    async def get(
        self, audit_id: uuid.UUID, request_context: RequestContext
    ) -> DisplayAudit | None:
        async with self._uow_factory() as uow:
            audit = await DisplayAuditRepository(uow.session).get(audit_id)
            if audit is None:
                return None

            visit = await VisitRepository(uow.session).get(audit.visit_id)
            assert visit is not None, "DisplayAudit.visit_id is ON DELETE RESTRICT"
            await self._authorize(visit, request_context)

            uow.session.expunge(audit)
            return audit

    async def list(self, request_context: RequestContext) -> Sequence[DisplayAudit]:
        """DisplayAudit rows whose parent Visit belongs to the caller's own
        `employee_id`.

        Loaded via `DisplayAuditRepository.list()` plus a per-row
        parent-Visit lookup, mirroring `PosmAuditService.list` exactly --
        `DisplayAudit` has no `employee_id` column to filter on directly.
        """
        async with self._uow_factory() as uow:
            audits = await DisplayAuditRepository(uow.session).list()
            visit_repo = VisitRepository(uow.session)
            current_employee_id = request_context.employee_context.employee.id

            owned: list[DisplayAudit] = []
            for audit in audits:
    ...
    async def list_paginated(
        ...
        *,
        ...
        `DisplayAuditRepository.paginate_by_employee_id` -- in-memory
        ...
        async with self._uow_factory() as uow:
            current_employee_id = request_context.employee_context.employee.id
            page = await DisplayAuditRepository(uow.session).paginate_by_employee_id(
                current_employee_id,
    ...
    async def _authorize(self, resource: Any, request_context: RequestContext) -> None:
        ...
        `resource`, which is always the DisplayAudit's parent `Visit` --
        never the DisplayAudit itself. `VisitAuthorizationEvaluator` is
        reused completely unmodified: it only ever inspects
        ...
        if not decision.allowed:
            raise DisplayAuditAuthorizationDeniedError(
                decision.reason or "Display audit authorization denied"
            )
