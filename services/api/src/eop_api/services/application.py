...
class ApplicationService:
    ...
    Mirrors `AssignmentService`'s structure exactly: existence checks for
    both referenced aggregates, pair-uniqueness enforcement, no dedicated
    authorization evaluator. Unlike `AssignmentService`, no organization-
    mismatch check is added -- `Candidate` is not organization-scoped in
    this iteration, so there is nothing to compare `JobRequisition
    .organization_id` against (`iteration-1-scope-and-implementation-plan.md`
    §2).
    ...
    async def create(self, data: ApplicationCreate) -> Application:
        async with self._uow_factory() as uow:
            ...
            application = await repo.create(**data.model_dump())
            await uow.commit()
