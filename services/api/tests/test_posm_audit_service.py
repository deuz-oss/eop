...
@pytest.fixture
def service(session_factory: Callable[[], AsyncSession]) -> PosmAuditService:
    uow_factory: Callable[[], SQLAlchemyUnitOfWork] = lambda: SQLAlchemyUnitOfWork(  # noqa: E731
        session_factory
    )
    return PosmAuditService(uow_factory)
...
def _request_context(employee_id: uuid.UUID) -> RequestContext:
    ...
    `VisitAuthorizationEvaluator`/`PosmAuditService`, so the `User`/
    `HrEmployee` here need not be persisted -- mirrors
    `test_competitor_activity_service.py`'s exact pattern.
    """
    user = User(
        id=uuid.uuid4(),
...
def _create(visit_id: uuid.UUID, **overrides) -> PosmAuditCreate:
    values = {
        "visit_id": visit_id,
        "posm_type": "Banner",
    ...
    return PosmAuditCreate(**values)


async def test_create_and_get(
    service: PosmAuditService, employee_id: uuid.UUID, visit_id: uuid.UUID
):
...
async def test_create_rejects_missing_visit(service: PosmAuditService, employee_id: uuid.UUID):
...
async def test_create_denied_for_non_owner(
    service: PosmAuditService,
    employee_id: uuid.UUID,
    other_employee_id: uuid.UUID,
    visit_id: uuid.UUID,
):
    with pytest.raises(PosmAuditAuthorizationDeniedError):
...
async def test_create_allows_multiple_per_visit(
    ...
    context = _request_context(employee_id)
    first = await service.create(_create(visit_id, posm_type="Banner"), context)
    second = await service.create(_create(visit_id, posm_type="Wobbler"), context)

    assert first.id != second.id


async def test_get_missing_returns_none(service: PosmAuditService, employee_id: uuid.UUID):
...
async def test_update_existing(
    ...
    updated = await service.update(audit.id, PosmAuditUpdate(notes="Updated"), context)
...
async def test_update_missing_returns_none(service: PosmAuditService, employee_id: uuid.UUID):
    assert (
        await service.update(
            uuid.uuid4(), PosmAuditUpdate(notes="x"), _request_context(employee_id)
        )
        is None
    )
...
async def test_update_denied_for_non_owner(
    ...
    with pytest.raises(PosmAuditAuthorizationDeniedError):
        await service.update(
            audit.id, PosmAuditUpdate(notes="x"), _request_context(other_employee_id)
        )
...
async def test_delete_missing_returns_false(service: PosmAuditService, employee_id: uuid.UUID):
