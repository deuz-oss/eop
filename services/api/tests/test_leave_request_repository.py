...
@pytest.fixture
def repo(session: AsyncSession) -> LeaveRequestRepository:
...
async def employee_id(session: AsyncSession) -> uuid.UUID:
    ...
    LeaveRequest only needs a valid `HrEmployee.id` to point at -- the full
    ...
    requires it, not because LeaveRequest cares about any of it.
    """
...
async def test_create_and_get(repo: LeaveRequestRepository, employee_id: uuid.UUID):
    ...
    fetched = await repo.get(leave_request.id)
...
async def test_get_missing_returns_none(repo: LeaveRequestRepository):
    assert await repo.get(uuid.uuid4()) is None


async def test_list_returns_created(repo: LeaveRequestRepository, employee_id: uuid.UUID):
    await repo.create(
...
async def test_update_existing(repo: LeaveRequestRepository, employee_id: uuid.UUID):
    leave_request = await repo.create(
        employee_id=employee_id, start_date=date(2026, 2, 10), end_date=date(2026, 2, 12)
    )

    updated = await repo.update(leave_request.id, status="approved")

    assert updated is not None
    assert updated.status == "approved"


async def test_update_missing_returns_none(repo: LeaveRequestRepository):
    assert await repo.update(uuid.uuid4(), status="approved") is None


async def test_delete_existing(repo: LeaveRequestRepository, employee_id: uuid.UUID):
    leave_request = await repo.create(
        employee_id=employee_id, start_date=date(2026, 2, 10), end_date=date(2026, 2, 12)
    )

    deleted = await repo.delete(leave_request.id)

    assert deleted is True
    assert await repo.get(leave_request.id) is None


async def test_delete_missing_returns_false(repo: LeaveRequestRepository):
    assert await repo.delete(uuid.uuid4()) is False


async def test_delete_employee_referenced_by_leave_request_is_restricted(
...
async def test_paginate_defaults(repo: LeaveRequestRepository, employee_id: uuid.UUID):
    for i in range(3):
        await repo.create(
            employee_id=employee_id,
            start_date=date(2026, 2, 1 + i),
...
async def test_paginate_search_returns_matching_rows_by_reason(
    ...
    page = await repo.paginate(search=SearchParams(q="vacation"))
...
async def test_paginate_filters_by_status(repo: LeaveRequestRepository, employee_id: uuid.UUID):
...
async def test_paginate_filters_by_employee_id(
    ...
    assert page.total == 1
    assert page.items[0].id == leave_request.id
