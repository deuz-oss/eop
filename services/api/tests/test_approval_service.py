...
@pytest.fixture
async def approver_id(session_factory: Callable[[], AsyncSession]) -> uuid.UUID:
    async with session_factory() as session:
...
async def test_approve_leave_request(
    ...
    approved = await service.approve_leave_request(leave_request_id, approver_id)
...
async def test_reject_leave_request(
    ...
    rejected = await service.reject_leave_request(
        leave_request_id, approver_id, "Insufficient coverage"
    )

    assert rejected is not None
    assert rejected.status == "rejected"
    assert rejected.approved_by == approver_id
...
async def test_approve_leave_request_missing_returns_none(
    service: ApprovalService, approver_id: uuid.UUID
):
    assert await service.approve_leave_request(uuid.uuid4(), approver_id) is None


async def test_reject_leave_request_missing_returns_none(
    service: ApprovalService, approver_id: uuid.UUID
):
    assert await service.reject_leave_request(uuid.uuid4(), approver_id, "No") is None
...
async def test_approve_leave_request_rejects_non_pending(
    ...
    leave_request_id = await _leave_request_id(leave_request_service, employee_id)
    await service.approve_leave_request(leave_request_id, approver_id)
...
async def test_reject_leave_request_rejects_non_pending(
    ...
    leave_request_id = await _leave_request_id(leave_request_service, employee_id)
    await service.reject_leave_request(leave_request_id, approver_id, "No")
...
async def test_approve_overtime_request(
    ...
    approved = await service.approve_overtime_request(overtime_request_id, approver_id)
...
async def test_reject_overtime_request(
    ...
    rejected = await service.reject_overtime_request(
        overtime_request_id, approver_id, "Insufficient coverage"
    )
...
async def test_approve_overtime_request_missing_returns_none(
    ...
    assert await service.approve_overtime_request(uuid.uuid4(), approver_id) is None


async def test_reject_overtime_request_missing_returns_none(
    service: ApprovalService, approver_id: uuid.UUID
):
    assert await service.reject_overtime_request(uuid.uuid4(), approver_id, "No") is None
...
async def test_approve_overtime_request_rejects_non_pending(
    ...
    overtime_request_id = await _overtime_request_id(overtime_request_service, employee_id)
    await service.approve_overtime_request(overtime_request_id, approver_id)
...
async def test_reject_overtime_request_rejects_non_pending(
    ...
    overtime_request_id = await _overtime_request_id(overtime_request_service, employee_id)
    await service.reject_overtime_request(overtime_request_id, approver_id, "No")
...
async def test_approve_timesheet(
    ...
    approved = await service.approve_timesheet(timesheet_id, approver_id)
...
async def test_reject_timesheet(
    ...
    rejected = await service.reject_timesheet(timesheet_id, approver_id, "Insufficient coverage")
...
async def test_approve_timesheet_missing_returns_none(
    ...
    assert await service.approve_timesheet(uuid.uuid4(), approver_id) is None


async def test_reject_timesheet_missing_returns_none(
    service: ApprovalService, approver_id: uuid.UUID
):
    assert await service.reject_timesheet(uuid.uuid4(), approver_id, "No") is None
...
async def test_approve_timesheet_rejects_non_pending(
    ...
    timesheet_id = await _timesheet_id(timesheet_service, employee_id)
    await service.approve_timesheet(timesheet_id, approver_id)
...
async def test_reject_timesheet_rejects_non_pending(
    ...
    timesheet_id = await _timesheet_id(timesheet_service, employee_id)
    await service.reject_timesheet(timesheet_id, approver_id, "No")
