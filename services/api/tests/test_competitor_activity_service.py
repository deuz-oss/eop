...
@pytest.fixture
def service(session_factory: Callable[[], AsyncSession]) -> CompetitorActivityService:
    uow_factory: Callable[[], SQLAlchemyUnitOfWork] = lambda: SQLAlchemyUnitOfWork(  # noqa: E731
        session_factory
    )
    return CompetitorActivityService(uow_factory)
...
def _request_context(employee_id: uuid.UUID) -> RequestContext:
    ...
    `VisitAuthorizationEvaluator`/`CompetitorActivityService`, so the
    `User`/`HrEmployee` here need not be persisted -- mirrors
    `test_survey_service.py`'s exact pattern.
    """
    user = User(
        id=uuid.uuid4(),
...
def _create(visit_id: uuid.UUID, **overrides) -> CompetitorActivityCreate:
    values = {
        "visit_id": visit_id,
    ...
    return CompetitorActivityCreate(**values)


async def test_create_and_get(
    service: CompetitorActivityService, employee_id: uuid.UUID, visit_id: uuid.UUID
):
    context = _request_context(employee_id)
    activity = await service.create(_create(visit_id), context)

    fetched = await service.get(activity.id, context)
...
async def test_create_rejects_missing_visit(
    service: CompetitorActivityService, employee_id: uuid.UUID
):
...
async def test_create_denied_for_non_owner(
    service: CompetitorActivityService,
    employee_id: uuid.UUID,
    other_employee_id: uuid.UUID,
    visit_id: uuid.UUID,
):
    with pytest.raises(CompetitorActivityAuthorizationDeniedError):
...
async def test_create_allows_multiple_per_visit(
    ...
    context = _request_context(employee_id)
    first = await service.create(_create(visit_id, competitor_name="Competitor A"), context)
    second = await service.create(_create(visit_id, competitor_name="Competitor B"), context)

    assert first.id != second.id


async def test_get_missing_returns_none(service: CompetitorActivityService, employee_id: uuid.UUID):
...
async def test_get_denied_for_non_owner(
    ...
    activity = await service.create(_create(visit_id), owner_context)

    with pytest.raises(CompetitorActivityAuthorizationDeniedError):
        await service.get(activity.id, _request_context(other_employee_id))
...
async def test_list_by_visit_id_returns_multiple(
    ...
    found = await service.list_by_visit_id(visit_id, context)

    assert {item.id for item in found} == {first.id, second.id}


async def test_list_by_visit_id_rejects_missing_visit(
    service: CompetitorActivityService, employee_id: uuid.UUID
):
    with pytest.raises(VisitNotFoundError):
        await service.list_by_visit_id(uuid.uuid4(), _request_context(employee_id))
...
async def test_update_existing(
    ...
    updated = await service.update(activity.id, CompetitorActivityUpdate(notes="Updated"), context)
...
async def test_update_missing_returns_none(
    ...
    assert (
        await service.update(
            uuid.uuid4(), CompetitorActivityUpdate(notes="x"), _request_context(employee_id)
        )
        is None
    )
...
async def test_update_denied_for_non_owner(
    ...
    with pytest.raises(CompetitorActivityAuthorizationDeniedError):
        await service.update(
            activity.id, CompetitorActivityUpdate(notes="x"), _request_context(other_employee_id)
        )
...
async def test_delete_existing(
    ...
    deleted = await service.delete(activity.id, context)

    assert deleted is True
    assert await service.get(activity.id, context) is None
...
async def test_delete_denied_for_non_owner(
    ...
    with pytest.raises(CompetitorActivityAuthorizationDeniedError):
        await service.delete(activity.id, _request_context(other_employee_id))

    assert await service.get(activity.id, owner_context) is not None
...
async def test_authorization_follows_current_visit_owner(
    ...
    other_context = _request_context(other_employee_id)
    fetched = await service.get(activity.id, other_context)
    assert fetched is not None

    with pytest.raises(CompetitorActivityAuthorizationDeniedError):
        await service.get(activity.id, owner_context)
