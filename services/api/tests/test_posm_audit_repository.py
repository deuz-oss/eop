...
@pytest.fixture
def repo(session: AsyncSession) -> PosmAuditRepository:
    return PosmAuditRepository(session)
...
async def test_create_and_get(repo: PosmAuditRepository, visit_id: uuid.UUID):
    ...
    assert fetched.posm_type == "Banner"
    assert fetched.condition == "Installed, good condition"
    assert fetched.notes == "Near entrance"


async def test_get_missing_returns_none(repo: PosmAuditRepository):
    assert await repo.get(uuid.uuid4()) is None


async def test_multiple_audits_per_visit_allowed(repo: PosmAuditRepository, visit_id: uuid.UUID):
    first = await repo.create(visit_id=visit_id, posm_type="Banner", condition="Good")
    second = await repo.create(visit_id=visit_id, posm_type="Banner", condition="Good")

    assert first.id != second.id


async def test_update_existing(repo: PosmAuditRepository, visit_id: uuid.UUID):
    audit = await repo.create(visit_id=visit_id, posm_type="Banner", condition="Good")

    updated = await repo.update(audit.id, notes="Updated note")

    assert updated is not None
    assert updated.notes == "Updated note"
    assert updated.posm_type == "Banner"


async def test_delete_existing(repo: PosmAuditRepository, visit_id: uuid.UUID):
    audit = await repo.create(visit_id=visit_id, posm_type="Banner", condition="Good")
...
async def test_delete_missing_returns_false(repo: PosmAuditRepository):
    assert await repo.delete(uuid.uuid4()) is False


async def test_paginate_filters_by_visit_id(repo: PosmAuditRepository, visit_id: uuid.UUID):
    await repo.create(visit_id=visit_id, posm_type="Banner", condition="Good")
    await repo.create(visit_id=visit_id, posm_type="Wobbler", condition="Damaged")
...
async def test_paginate_returns_total_and_page_slice(
    repo: PosmAuditRepository, visit_id: uuid.UUID
):
    for i in range(5):
        await repo.create(visit_id=visit_id, posm_type=f"POSM {i}", condition="Good")
