...
async def test_get_by_employee_id(repo: CompensationRepository, employee_id: uuid.UUID):
    compensation = await repo.create(
        employee_id=employee_id,
        base_salary_amount=Decimal("1000.00"),
        base_salary_currency="IDR",
    ...
    found = await repo.get_by_employee_id(employee_id)

    assert found is not None
    assert found.id == compensation.id
    assert await repo.get_by_employee_id(uuid.uuid4()) is None
