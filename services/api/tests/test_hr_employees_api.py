...
def _tables() -> Generator[None]:
    ...
    and `location_types` don't depend on `organizations`, so they're
    ...
    async def _truncate() -> None:
        ...
        async with engine.begin() as conn:
            await conn.execute(
                text("TRUNCATE TABLE organizations, locations, location_types, users CASCADE")
            )
        await engine.dispose()
...
class _Refs:
    def __init__(
        self, organization: dict, department: dict, position: dict, team: dict, location: dict
    ):
...
def _create_refs(client: TestClient, headers: dict[str, str], *, suffix: str = "") -> _Refs:
    ...
    return _Refs(organization, department, position, team, location)
