...
@pytest.fixture
def repo(session: AsyncSession) -> DisplayAuditRepository:
    return DisplayAuditRepository(session)
...
async def test_create_and_get(repo: DisplayAuditRepository, visit_id: uuid.UUID):
    audit = await repo.create(
        visit_id=visit_id,
        display_area="Main Shelf",
        observation="Products aligned per standard",
        notes="Near entrance",
    )

    fetched = await repo.get(audit.id)

    assert fetched is not None
    assert fetched.visit_id == visit_id
    assert fetched.display_area == "Main Shelf"
    assert fetched.observation == "Products aligned per standard"
    assert fetched.notes == "Near entrance"


async def test_get_missing_returns_none(repo: DisplayAuditRepository):
    assert await repo.get(uuid.uuid4()) is None


async def test_multiple_audits_per_visit_allowed(repo: DisplayAuditRepository, visit_id: uuid.UUID):
    first = await repo.create(visit_id=visit_id, display_area="Main Shelf", observation="Compliant")
    second = await repo.create(
        visit_id=visit_id, display_area="Main Shelf", observation="Compliant"
    )

    assert first.id != second.id


async def test_update_existing(repo: DisplayAuditRepository, visit_id: uuid.UUID):
    audit = await repo.create(visit_id=visit_id, display_area="Main Shelf", observation="Compliant")

    updated = await repo.update(audit.id, notes="Updated note")
...
async def test_delete_existing(repo: DisplayAuditRepository, visit_id: uuid.UUID):
    audit = await repo.create(visit_id=visit_id, display_area="Main Shelf", observation="Compliant")

    deleted = await repo.delete(audit.id)

    assert deleted is True
    assert await repo.get(audit.id) is None


async def test_delete_missing_returns_false(repo: DisplayAuditRepository):
    assert await repo.delete(uuid.uuid4()) is False


async def test_paginate_filters_by_visit_id(repo: DisplayAuditRepository, visit_id: uuid.UUID):
    await repo.create(visit_id=visit_id, display_area="Main Shelf", observation="Compliant")
    await repo.create(visit_id=visit_id, display_area="Window Display", observation="Damaged")
...
async def test_paginate_returns_total_and_page_slice(
    repo: DisplayAuditRepository, visit_id: uuid.UUID
):
    for i in range(5):
        await repo.create(visit_id=visit_id, display_area=f"Area {i}", observation="Compliant")
