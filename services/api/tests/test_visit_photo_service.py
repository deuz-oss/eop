        ...
        async with engine.begin() as conn:
            await conn.execute(
                text(
                    ...
                    "store_types, file_objects CASCADE"
                )
            )
...
@pytest.fixture
def service(session_factory: Callable[[], AsyncSession]) -> VisitPhotoService:
    uow_factory: Callable[[], SQLAlchemyUnitOfWork] = lambda: SQLAlchemyUnitOfWork(  # noqa: E731
        session_factory
    )
    return VisitPhotoService(uow_factory)
...
@pytest.fixture
async def file_object_id(session_factory: Callable[[], AsyncSession]) -> uuid.UUID:
    async with session_factory() as session:
        file_object = await FileRepository(session).create(
            filename="photo.jpg",
            content_type="image/jpeg",
            size=2048,
            storage_key=f"visit-photos/{uuid.uuid4().hex}.jpg",
            bucket="eop-files",
        )
        await session.commit()
...
def _request_context(employee_id: uuid.UUID) -> RequestContext:
    ...
    `VisitAuthorizationEvaluator`/`VisitPhotoService`, so the `User`/
...
def _create(visit_id: uuid.UUID, file_object_id: uuid.UUID, **overrides) -> VisitPhotoCreate:
    values = {"visit_id": visit_id, "file_object_id": file_object_id}
    values.update(overrides)
    return VisitPhotoCreate(**values)


async def test_create_and_get(
    service: VisitPhotoService,
    employee_id: uuid.UUID,
    visit_id: uuid.UUID,
    file_object_id: uuid.UUID,
):
    context = _request_context(employee_id)
    photo = await service.create(_create(visit_id, file_object_id), context)

    fetched = await service.get(photo.id, context)

    assert fetched is not None
    assert fetched.visit_id == visit_id
    assert fetched.file_object_id == file_object_id


async def test_create_rejects_missing_visit(
    service: VisitPhotoService, employee_id: uuid.UUID, file_object_id: uuid.UUID
):
    with pytest.raises(VisitNotFoundError):
        await service.create(_create(uuid.uuid4(), file_object_id), _request_context(employee_id))


async def test_create_rejects_missing_file_object(
    service: VisitPhotoService, employee_id: uuid.UUID, visit_id: uuid.UUID
):
    with pytest.raises(FileObjectNotFoundError):
        await service.create(_create(visit_id, uuid.uuid4()), _request_context(employee_id))
...
async def test_create_denied_for_non_owner(
    ...
    with pytest.raises(VisitPhotoAuthorizationDeniedError):
        await service.create(_create(visit_id, file_object_id), _request_context(other_employee_id))
...
async def test_create_allows_multiple_per_visit(
    ...
    context = _request_context(employee_id)
    async with session_factory() as session:
        other_file = await FileRepository(session).create(
            filename="photo2.jpg",
            content_type="image/jpeg",
            size=2048,
            storage_key=f"visit-photos/{uuid.uuid4().hex}.jpg",
    ...
    first = await service.create(_create(visit_id, file_object_id), context)
    second = await service.create(_create(visit_id, other_file_id), context)

    assert first.id != second.id


async def test_get_missing_returns_none(service: VisitPhotoService, employee_id: uuid.UUID):
...
async def test_get_denied_for_non_owner(
    ...
    photo = await service.create(_create(visit_id, file_object_id), owner_context)

    with pytest.raises(VisitPhotoAuthorizationDeniedError):
        await service.get(photo.id, _request_context(other_employee_id))
...
async def test_list_returns_only_owned(
    ...
    await service.create(_create(visit_id, file_object_id), context)
    await service.create(_create(other_visit_id, file_object_id), other_context)
...
async def test_update_existing(
    ...
    updated = await service.update(
        photo.id, VisitPhotoUpdate(file_object_id=other_file_id), context
    )

    assert updated is not None
    assert updated.file_object_id == other_file_id


async def test_update_missing_returns_none(service: VisitPhotoService, employee_id: uuid.UUID):
    assert (
        await service.update(
            uuid.uuid4(),
            VisitPhotoUpdate(file_object_id=uuid.uuid4()),
...
async def test_delete_existing(
    ...
    deleted = await service.delete(photo.id, context)

    assert deleted is True
    assert await service.get(photo.id, context) is None


async def test_delete_missing_returns_false(service: VisitPhotoService, employee_id: uuid.UUID):
...
async def test_delete_denied_for_non_owner(
    ...
    with pytest.raises(VisitPhotoAuthorizationDeniedError):
        await service.delete(photo.id, _request_context(other_employee_id))

    assert await service.get(photo.id, owner_context) is not None
...
async def test_authorization_follows_current_visit_owner(
    ...
    other_context = _request_context(other_employee_id)
    fetched = await service.get(photo.id, other_context)
    assert fetched is not None

    with pytest.raises(VisitPhotoAuthorizationDeniedError):
        await service.get(photo.id, owner_context)
