...
@pytest.fixture
def service(session_factory: Callable[[], AsyncSession]) -> SurveyService:
    uow_factory: Callable[[], SQLAlchemyUnitOfWork] = lambda: SQLAlchemyUnitOfWork(  # noqa: E731
        session_factory
    )
    return SurveyService(uow_factory)
...
def _request_context(employee_id: uuid.UUID) -> RequestContext:
    ...
    `VisitAuthorizationEvaluator`/`SurveyService`, so the `User`/`HrEmployee`
    here need not be persisted -- mirrors `test_visit_service.py`'s exact
    pattern.
    """
    user = User(
        id=uuid.uuid4(),
...
def _create(visit_id: uuid.UUID, **overrides) -> SurveyCreate:
    values = {
        "visit_id": visit_id,
    ...
    return SurveyCreate(**values)


async def test_create_and_get(service: SurveyService, employee_id: uuid.UUID, visit_id: uuid.UUID):
    context = _request_context(employee_id)
    survey = await service.create(_create(visit_id), context)

    fetched = await service.get(survey.id, context)

    assert fetched is not None
    assert fetched.visit_id == visit_id
    assert fetched.display_compliant is True


async def test_create_rejects_missing_visit(service: SurveyService, employee_id: uuid.UUID):
...
async def test_create_denied_for_non_owner(
    ...
    with pytest.raises(SurveyAuthorizationDeniedError):
        await service.create(_create(visit_id), _request_context(other_employee_id))


async def test_create_rejects_duplicate_visit(
    service: SurveyService, employee_id: uuid.UUID, visit_id: uuid.UUID
):
    context = _request_context(employee_id)
    await service.create(_create(visit_id), context)

    with pytest.raises(DuplicateSurveyError):
        await service.create(_create(visit_id), context)


async def test_get_missing_returns_none(service: SurveyService, employee_id: uuid.UUID):
...
async def test_get_denied_for_non_owner(
    ...
    survey = await service.create(_create(visit_id), owner_context)

    with pytest.raises(SurveyAuthorizationDeniedError):
        await service.get(survey.id, _request_context(other_employee_id))


async def test_get_by_visit_id(service: SurveyService, employee_id: uuid.UUID, visit_id: uuid.UUID):
    context = _request_context(employee_id)
    survey = await service.create(_create(visit_id), context)

    found = await service.get_by_visit_id(visit_id, context)
...
async def test_update_existing(service: SurveyService, employee_id: uuid.UUID, visit_id: uuid.UUID):
    context = _request_context(employee_id)
    survey = await service.create(_create(visit_id), context)

    updated = await service.update(survey.id, SurveyUpdate(stock_available=False), context)

    assert updated is not None
    assert updated.stock_available is False
    assert updated.display_compliant is True


async def test_update_missing_returns_none(service: SurveyService, employee_id: uuid.UUID):
    assert (
        await service.update(
            uuid.uuid4(), SurveyUpdate(stock_available=False), _request_context(employee_id)
        )
        is None
    )
...
async def test_update_denied_for_non_owner(
    ...
    with pytest.raises(SurveyAuthorizationDeniedError):
        await service.update(
            survey.id, SurveyUpdate(stock_available=False), _request_context(other_employee_id)
        )


async def test_delete_existing(service: SurveyService, employee_id: uuid.UUID, visit_id: uuid.UUID):
    context = _request_context(employee_id)
    survey = await service.create(_create(visit_id), context)

    deleted = await service.delete(survey.id, context)

    assert deleted is True
    assert await service.get(survey.id, context) is None


async def test_delete_missing_returns_false(service: SurveyService, employee_id: uuid.UUID):
...
async def test_delete_denied_for_non_owner(
    ...
    with pytest.raises(SurveyAuthorizationDeniedError):
        await service.delete(survey.id, _request_context(other_employee_id))

    assert await service.get(survey.id, owner_context) is not None
...
async def test_authorization_follows_current_visit_owner(
    ...
    since `Survey` carries no `employee_id` of its own (§5)."""
    owner_context = _request_context(employee_id)
    survey = await service.create(_create(visit_id), owner_context)
    ...
    other_context = _request_context(other_employee_id)
    fetched = await service.get(survey.id, other_context)
    assert fetched is not None

    with pytest.raises(SurveyAuthorizationDeniedError):
        await service.get(survey.id, owner_context)
