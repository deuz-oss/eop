...
@pytest.fixture
def repo(session: AsyncSession) -> VisitPhotoRepository:
    return VisitPhotoRepository(session)
...
@pytest.fixture
async def file_object_id(session: AsyncSession) -> uuid.UUID:
...
async def test_create_and_get(
    repo: VisitPhotoRepository, visit_id: uuid.UUID, file_object_id: uuid.UUID
):
    photo = await repo.create(visit_id=visit_id, file_object_id=file_object_id)

    fetched = await repo.get(photo.id)
...
async def test_get_missing_returns_none(repo: VisitPhotoRepository):
    assert await repo.get(uuid.uuid4()) is None


async def test_multiple_photos_per_visit_allowed(
    repo: VisitPhotoRepository,
    visit_id: uuid.UUID,
    ...
    first = await repo.create(visit_id=visit_id, file_object_id=file_object_id)
    second = await repo.create(visit_id=visit_id, file_object_id=other_file.id)
...
async def test_update_existing(
    ...
    updated = await repo.update(photo.id, file_object_id=other_file.id)

    assert updated is not None
    assert updated.file_object_id == other_file.id
...
async def test_delete_existing(
    ...
    deleted = await repo.delete(photo.id)

    assert deleted is True
    assert await repo.get(photo.id) is None


async def test_delete_missing_returns_false(repo: VisitPhotoRepository):
...
async def test_paginate_filters_by_visit_id(
    ...
    await repo.create(visit_id=visit_id, file_object_id=file_object_id)
    await repo.create(visit_id=visit_id, file_object_id=other_file.id)
...
async def test_paginate_returns_total_and_page_slice(
    repo: VisitPhotoRepository, visit_id: uuid.UUID, session: AsyncSession
):
    for i in range(5):
        file_object = await FileRepository(session).create(
        ...
        await repo.create(visit_id=visit_id, file_object_id=file_object.id)
