...
@pytest.fixture
def other() -> User:
    """An authenticated user who does not own the visit/display audit under
    test."""
...
def admin_user() -> User:
    ...
    Visit/DisplayAudit's own Owner Only policy -- this user only exists to
...
def _create_audit(client: TestClient, headers: dict[str, str], visit_id: str, **overrides) -> dict:
    response = client.post(
        "/display-audits", json=_audit_payload(visit_id, **overrides), headers=headers
    )
    assert response.status_code == 201
    return response.json()
...
def test_create_audit_requires_authentication(client: TestClient):
    response = client.post("/display-audits", json=_audit_payload(str(uuid.uuid4())))
    assert response.status_code == 401


def test_list_audits_requires_authentication(client: TestClient):
    assert client.get("/display-audits").status_code == 401


def test_get_audit_requires_authentication(client: TestClient):
    assert client.get(f"/display-audits/{uuid.uuid4()}").status_code == 401


def test_update_audit_requires_authentication(client: TestClient):
    response = client.put(f"/display-audits/{uuid.uuid4()}", json={"notes": "x"})
    assert response.status_code == 401


def test_delete_audit_requires_authentication(client: TestClient):
    assert client.delete(f"/display-audits/{uuid.uuid4()}").status_code == 401
...
def test_create_audit(
    ...
    assert body["display_area"] == "Main Shelf"
    assert body["observation"] == "Compliant"
...
def test_create_audit_rejects_missing_visit(
    ...
    response = client.post(
        "/display-audits", json=_audit_payload(str(uuid.uuid4())), headers=user_headers
    )

    assert response.status_code == 404


def test_create_audit_rejects_missing_fields(
    client: TestClient, user: User, user_headers: dict[str, str], admin_headers: dict[str, str]
):
    employee = _create_employee(client, user_headers, user_id=str(user.id))
    store = _create_store(client, admin_headers)
    ...
    response = client.post(
        "/display-audits",
        json={"visit_id": visit["id"], "display_area": "", "observation": "Compliant"},
...
def test_create_audit_allows_multiple_per_visit(
    ...
    store = _create_store(client, admin_headers)
    visit = _create_visit(client, user_headers, employee["id"], store["id"])
    _create_audit(client, user_headers, visit["id"], display_area="Main Shelf")

    response = client.post(
        "/display-audits",
        json=_audit_payload(visit["id"], display_area="Window Display"),
        headers=user_headers,
    )
...
def test_create_audit_forbidden_for_non_owner(
    ...
    response = client.post(
        "/display-audits", json=_audit_payload(visit["id"]), headers=other_headers
    )
...
def test_get_audit(
    ...
    response = client.get(f"/display-audits/{created['id']}", headers=user_headers)
...
def test_get_audit_not_found(client: TestClient, user: User, user_headers: dict[str, str]):
    ...
    response = client.get(f"/display-audits/{uuid.uuid4()}", headers=user_headers)
...
def test_get_audit_forbidden(
    ...
    response = client.get(f"/display-audits/{created['id']}", headers=other_headers)
...
def test_list_audits_returns_only_owned(
    ...
    response = client.get("/display-audits", headers=user_headers)
...
def test_list_audits_paginated_returns_only_owned(
    ...
    response = client.get("/display-audits/paginated", headers=user_headers)
...
def test_list_audits_paginated_filters_by_visit_id(
    ...
    _create_audit(client, user_headers, visit["id"], display_area="Main Shelf")
    _create_audit(client, user_headers, other_visit["id"], display_area="Window Display")

    response = client.get(
        "/display-audits/paginated", params={"visit_id": visit["id"]}, headers=user_headers
    )
...
def test_update_audit(
    ...
    assert response.json()["display_area"] == "Main Shelf"
...
def test_update_audit_not_found(client: TestClient, user: User, user_headers: dict[str, str]):
    ...
    response = client.put(
        f"/display-audits/{uuid.uuid4()}", json={"notes": "x"}, headers=user_headers
    )
...
def test_update_audit_forbidden(
    ...
    response = client.put(
        f"/display-audits/{created['id']}", json={"notes": "x"}, headers=other_headers
    )
...
def test_delete_audit(
    ...
    response = client.delete(f"/display-audits/{created['id']}", headers=user_headers)

    assert response.status_code == 204
    assert client.get(f"/display-audits/{created['id']}", headers=user_headers).status_code == 404


def test_delete_audit_not_found(client: TestClient, user: User, user_headers: dict[str, str]):
    _create_employee(client, user_headers, user_id=str(user.id))

    response = client.delete(f"/display-audits/{uuid.uuid4()}", headers=user_headers)
    assert response.status_code == 404
...
def test_delete_audit_forbidden(
    ...
    response = client.delete(f"/display-audits/{created['id']}", headers=other_headers)
...
def test_audit_authorization_follows_current_visit_owner(
    ...
    assert client.get(f"/display-audits/{created['id']}", headers=other_headers).status_code == 200
    assert client.get(f"/display-audits/{created['id']}", headers=user_headers).status_code == 403
