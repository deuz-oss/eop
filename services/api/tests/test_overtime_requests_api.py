...
def test_approve_overtime_request_not_found(
...
def test_reject_overtime_request_not_found(
    client: TestClient, user: User, user_headers: dict[str, str]
):
    _create_employee(client, user_headers, user_id=str(user.id))
...
def test_approve_overtime_request(
    client: TestClient,
    ...
    response = client.post(
        f"/hr/overtime-requests/{created['id']}/approve", headers=manager_headers
    )
...
def test_reject_overtime_request(
    client: TestClient,
    user_headers: dict[str, str],
    manager: User,
    manager_headers: dict[str, str],
...
def test_approve_overtime_request_rejects_non_pending(
    ...
    _, requester = _create_manager_and_requester(client, user_headers, str(manager.id))
    created = _create_overtime_request(client, user_headers, requester["id"])
    client.post(f"/hr/overtime-requests/{created['id']}/approve", headers=manager_headers)
...
def test_approve_overtime_request_forbidden_for_non_manager(
    client: TestClient,
    user_headers: dict[str, str],
    manager: User,
    other: User,
...
def test_reject_overtime_request_forbidden_for_non_manager(
    client: TestClient,
    user_headers: dict[str, str],
    manager: User,
    other: User,
