...
def test_approve_overtime_request_not_found(client: TestClient, user_headers: dict[str, str]):
    response = client.post(f"/hr/overtime-requests/{uuid.uuid4()}/approve", headers=user_headers)

    assert response.status_code == 404


def test_reject_overtime_request_not_found(client: TestClient, user_headers: dict[str, str]):
    response = client.post(
...
def test_approve_overtime_request(client: TestClient, user_headers: dict[str, str]):
    employee = _create_employee(client, user_headers)
    created = _create_overtime_request(client, user_headers, employee["id"])

    response = client.post(
        f"/hr/overtime-requests/{created['id']}/approve", headers=user_headers
    )
...
def test_reject_overtime_request(client: TestClient, user_headers: dict[str, str]):
    employee = _create_employee(client, user_headers)
    created = _create_overtime_request(client, user_headers, employee["id"])

    response = client.post(
        f"/hr/overtime-requests/{created['id']}/reject",
...
def test_approve_overtime_request_rejects_non_pending(
    client: TestClient, user_headers: dict[str, str]
):
    employee = _create_employee(client, user_headers)
    created = _create_overtime_request(client, user_headers, employee["id"])
    client.post(f"/hr/overtime-requests/{created['id']}/approve", headers=user_headers)
...
def test_reject_overtime_request_rejects_non_pending(
    client: TestClient, user_headers: dict[str, str]
):
    employee = _create_employee(client, user_headers)
    created = _create_overtime_request(client, user_headers, employee["id"])
