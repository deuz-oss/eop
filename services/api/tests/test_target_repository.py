...
@pytest.fixture
def repo(session: AsyncSession) -> TargetRepository:
...
async def test_create_and_get(repo: TargetRepository, employee_id: uuid.UUID, kpi_id: uuid.UUID):
    ...
    fetched = await repo.get(target.id)
    ...
    assert fetched.goal_value == Decimal("95.5")


async def test_get_missing_returns_none(repo: TargetRepository):
    assert await repo.get(uuid.uuid4()) is None


async def test_get_by_identity(repo: TargetRepository, employee_id: uuid.UUID, kpi_id: uuid.UUID):
    target = await repo.create(
        employee_id=employee_id,
        kpi_id=kpi_id,
        period_year=2026,
    ...
    found = await repo.get_by_identity(employee_id, kpi_id, 2026, 8)

    assert found is not None
    assert found.id == target.id
    assert await repo.get_by_identity(employee_id, kpi_id, 2026, 9) is None


async def test_list_returns_created(
    repo: TargetRepository, employee_id: uuid.UUID, kpi_id: uuid.UUID
):
    await repo.create(
        employee_id=employee_id,
        kpi_id=kpi_id,
...
async def test_duplicate_identity_rejected_at_db_level(
    repo: TargetRepository, employee_id: uuid.UUID, kpi_id: uuid.UUID
):
    await repo.create(
        employee_id=employee_id,
    ...
    with pytest.raises(IntegrityError):
        await repo.create(
            ...
            period_year=2026,
            period_month=8,
            goal_value=Decimal("99"),
        )


async def test_update_existing(repo: TargetRepository, employee_id: uuid.UUID, kpi_id: uuid.UUID):
    target = await repo.create(
        employee_id=employee_id,
        kpi_id=kpi_id,
        period_year=2026,
    ...
    updated = await repo.update(target.id, goal_value=Decimal("42.25"))

    assert updated is not None
    assert updated.goal_value == Decimal("42.25")


async def test_delete_existing(repo: TargetRepository, employee_id: uuid.UUID, kpi_id: uuid.UUID):
    target = await repo.create(
        employee_id=employee_id,
        kpi_id=kpi_id,
        period_year=2026,
    ...
    deleted = await repo.delete(target.id)

    assert deleted is True
    assert await repo.get(target.id) is None


async def test_delete_missing_returns_false(repo: TargetRepository):
