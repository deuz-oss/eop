...
@pytest.fixture
def repo(session: AsyncSession) -> VisitRepository:
...
async def ids(session: AsyncSession) -> tuple[uuid.UUID, uuid.UUID]:
    ...
    FKs -- Visit only needs the two ids to point at, not the rest of the
    chain."""
...
async def test_create_and_get(repo: VisitRepository, ids: tuple[uuid.UUID, uuid.UUID]):
    employee_id, store_id = ids
    visited_at = datetime(2026, 1, 5, 9, 0, tzinfo=UTC)
    visit = await repo.create(
        employee_id=employee_id, store_id=store_id, visited_at=visited_at, notes="First visit"
    )

    fetched = await repo.get(visit.id)

    assert fetched is not None
    assert fetched.employee_id == employee_id
    assert fetched.store_id == store_id
    assert fetched.visited_at == visited_at
    assert fetched.notes == "First visit"


async def test_get_missing_returns_none(repo: VisitRepository):
    assert await repo.get(uuid.uuid4()) is None


async def test_multiple_visits_per_employee_and_store_allowed(
    repo: VisitRepository, ids: tuple[uuid.UUID, uuid.UUID]
):
    employee_id, store_id = ids
...
async def test_update_existing(repo: VisitRepository, ids: tuple[uuid.UUID, uuid.UUID]):
    employee_id, store_id = ids
    visit = await repo.create(
        employee_id=employee_id,
        store_id=store_id,
    ...
    updated = await repo.update(visit.id, notes="Updated")

    assert updated is not None
    assert updated.notes == "Updated"


async def test_delete_existing(repo: VisitRepository, ids: tuple[uuid.UUID, uuid.UUID]):
    employee_id, store_id = ids
    visit = await repo.create(
        employee_id=employee_id,
        store_id=store_id,
    ...
    deleted = await repo.delete(visit.id)

    assert deleted is True
    assert await repo.get(visit.id) is None


async def test_paginate_search_by_notes(repo: VisitRepository, ids: tuple[uuid.UUID, uuid.UUID]):
    employee_id, store_id = ids
    await repo.create(
        employee_id=employee_id,
        store_id=store_id,
    ...
    page = await repo.paginate(search=SearchParams(q="stock"))
...
async def test_paginate_filters_by_employee_id(
    ...
    assert page.total == 1
    assert page.items[0].id == visit.id
