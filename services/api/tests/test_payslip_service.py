...
async def test_create_and_get(
    ...
    fetched = await service.get(payslip.id)
...
async def test_get_missing_returns_none(service: PayslipService):
...
async def test_list_returns_created(
    ...
    await service.create(_create(employee_id, payroll_run_id))
    await service.create(_create(employee_id, payroll_run_id))
