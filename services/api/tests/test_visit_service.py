...
@pytest.fixture
def service(session_factory: Callable[[], AsyncSession]) -> VisitService:
    uow_factory: Callable[[], SQLAlchemyUnitOfWork] = lambda: SQLAlchemyUnitOfWork(  # noqa: E731
        session_factory
    )
    return VisitService(uow_factory)
...
@pytest.fixture
async def store_id(session_factory: Callable[[], AsyncSession]) -> uuid.UUID:
    async with session_factory() as session:
...
def _request_context(employee_id: uuid.UUID) -> RequestContext:
    ...
    `VisitAuthorizationEvaluator`/`VisitService`, so the `User`/`HrEmployee`
    here need not be persisted -- mirrors
    `test_attendance_event_service.py`'s exact pattern.
    """
    user = User(
        id=uuid.uuid4(),
...
def _create(employee_id: uuid.UUID, store_id: uuid.UUID, **overrides) -> VisitCreate:
    values = {
    ...
    return VisitCreate(**values)


async def test_create_and_get(service: VisitService, employee_id: uuid.UUID, store_id: uuid.UUID):
    context = _request_context(employee_id)
    visit = await service.create(_create(employee_id, store_id), context)

    fetched = await service.get(visit.id, context)
...
async def test_create_rejects_missing_employee(service: VisitService, store_id: uuid.UUID):
    with pytest.raises(EmployeeNotFoundError):
        await service.create(_create(uuid.uuid4(), store_id), _request_context(uuid.uuid4()))


async def test_create_rejects_missing_store(service: VisitService, employee_id: uuid.UUID):
...
async def test_create_denied_for_non_owner(
    service: VisitService, employee_id: uuid.UUID, other_employee_id: uuid.UUID, store_id: uuid.UUID
):
    with pytest.raises(VisitAuthorizationDeniedError):
        await service.create(_create(employee_id, store_id), _request_context(other_employee_id))


async def test_get_missing_returns_none(service: VisitService, employee_id: uuid.UUID):
...
async def test_get_denied_for_non_owner(
    ...
    visit = await service.create(_create(employee_id, store_id), owner_context)

    with pytest.raises(VisitAuthorizationDeniedError):
        await service.get(visit.id, _request_context(other_employee_id))


async def test_list_returns_created(
    service: VisitService, employee_id: uuid.UUID, store_id: uuid.UUID
):
    context = _request_context(employee_id)
    await service.create(_create(employee_id, store_id), context)
    await service.create(
        _create(employee_id, store_id, visited_at=datetime(2026, 1, 12, 9, 0, tzinfo=UTC)), context
    )
...
async def test_list_returns_only_owned(
    ...
    await service.create(_create(employee_id, store_id), context)
    await service.create(_create(other_employee_id, store_id), other_context)
...
async def test_update_existing(service: VisitService, employee_id: uuid.UUID, store_id: uuid.UUID):
    context = _request_context(employee_id)
    visit = await service.create(_create(employee_id, store_id), context)

    updated = await service.update(visit.id, VisitUpdate(notes="Updated"), context)

    assert updated is not None
    assert updated.notes == "Updated"


async def test_update_missing_returns_none(service: VisitService, employee_id: uuid.UUID):
    assert (
        await service.update(
            uuid.uuid4(), VisitUpdate(notes="Updated"), _request_context(employee_id)
        )
        is None
    )
...
async def test_update_rejects_missing_employee(
    ...
    with pytest.raises(EmployeeNotFoundError):
        await service.update(visit.id, VisitUpdate(employee_id=uuid.uuid4()), context)
...
async def test_update_rejects_missing_store(
    ...
    with pytest.raises(StoreNotFoundError):
        await service.update(visit.id, VisitUpdate(store_id=uuid.uuid4()), context)
...
async def test_update_denied_for_non_owner(
    ...
    with pytest.raises(VisitAuthorizationDeniedError):
        await service.update(
            visit.id, VisitUpdate(notes="Updated"), _request_context(other_employee_id)
        )


async def test_delete_existing(service: VisitService, employee_id: uuid.UUID, store_id: uuid.UUID):
    context = _request_context(employee_id)
    visit = await service.create(_create(employee_id, store_id), context)

    deleted = await service.delete(visit.id, context)

    assert deleted is True
    assert await service.get(visit.id, context) is None


async def test_delete_missing_returns_false(service: VisitService, employee_id: uuid.UUID):
...
async def test_delete_denied_for_non_owner(
    ...
    with pytest.raises(VisitAuthorizationDeniedError):
        await service.delete(visit.id, _request_context(other_employee_id))

    assert await service.get(visit.id, owner_context) is not None
...
async def test_list_paginated_passes_through_offset_and_limit(
    ...
    for i in range(5):
        await service.create(
            _create(employee_id, store_id, visited_at=datetime(2026, 1, 5, 9, i, tzinfo=UTC)),
            context,
        )
...
async def test_list_paginated_passes_through_search(
    ...
    context = _request_context(employee_id)
    await service.create(_create(employee_id, store_id, notes="Stock running low"), context)
    await service.create(
        _create(
            employee_id,
            store_id,
            visited_at=datetime(2026, 1, 12, 9, 0, tzinfo=UTC),
            notes="All good",
        ),
        context,
    )
    ...
    page = await service.list_paginated(
        context, PaginationParams(offset=0, limit=50), SearchParams(q="stock")
    )

    assert page.total == 1
    assert page.items[0].notes == "Stock running low"
